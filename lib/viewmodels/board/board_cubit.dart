...
class BoardCubit extends Cubit<BoardState> {
  ...
  Future<void> loadImageFile(String filePath) async {
    log("Rozpoczęcie ładowania pliku obrazu: $filePath");
    final shape = await _parseImageFile(filePath);
    if (shape != null) {
      emit(state.copyWith(
        drawingQueue: Queue.from(state.drawingQueue)..add(shape),
      ));
  ...
  Future<ImageShape?> _parsePNG(Uint8List content) async {
    try {
      final image = await decodeImageFromList(content);
      return ImageShape(
      ...
      log("Nie udało się zdekodować obrazu PNG: $e");
      return null;
    }
  }
  ...
  Future<ImageShape> _parseP3(Uint8List content) async {
    ...
    // Tworzymy obiekt obrazu z pakietu `image`
    ...
    for (int y = 0; y < height; y++) {
      for (int x = 0; x < width; x++) {
        ...
        image.setPixel(x, y, img.ColorUint32.rgb(r, g, b));
      }
    }

    // Kodowanie obrazu do formatu PNG
    final pngBytes = Uint8List.fromList(img.encodePng(image));

    // Konwertowanie PNG do `ui.Image`
    ...
    return ImageShape(
      type: ShapeType.freeDraw, // Typ obrazu
      startPosition: Offset.zero, // Startowa pozycja
      endPosition:
          Offset(width.toDouble(), height.toDouble()), // Pozycja końcowa
      image: uiImage, // Zwracamy ui.Image
    );
  }
